...
author-note:
  disclosures:
    ...
    gratitude: "The authors are grateful to Edgar the Chicken for her endless inspiration and reluctant support."
...
[paragraph about why bigger is better, with regards to egg production and everything else^[This paragraph did not end up getting written because really what more do we need to say.]]
...
### Chickens
```{r}
#| label: read_Gallus-Gallus_df
#| echo: FALSE 
#| include: FALSE

## Reading the data 
Gallus_Gallus = read.csv("Data_BigAssChicken/GallusGallusDomesticus.csv")

```
We looked at a total of **`r n_distinct(Gallus_Gallus$GallusID)`** Gallus gallus domesticus chicks, starting from hatching.

### Eggs
```{r}
#| label: chicken_egg_data
#| echo: FALSE 
#| include: FALSE
...
breeds <- c("Leghorn", "Rhode Island Red", "Orpington", "Sussex", "Australorp") # my chickens are orpingtons so i included them 
...
weights <- seq(2.5, 6.5, by = 0.1)  # Chicken weights from 2.5 kg to 6.5 kg - the 6.5 kg is what my chickens weight so that waht wen with 
...
#  egg size based on weight (heavier chickens lay larger eggs)
...
```  
We looked at a total of **`r n_distinct(chicken_data$Eggs_Laid)`** eggs laid from **`r n_distinct(chicken_data$Chicken_ID)`** chickens.

### Humans
```{r}
#| label: urban-chicken-keepers_df
#| echo: FALSE 
#| include: FALSE
# Creating human data frame:
...
# Generate gender data (32 female, 17 male, 1 non-binary)
gender <- c(rep("Female", 32), rep("Male", 17), rep("Non-binary", 1))

# Generate location data (20 Eastern Seaboard, 15 Midwest, 15 Pacific Northwest)
location <- c(rep("Eastern Seaboard", 20), rep("Midwest", 15), rep("Pacific Northwest", 15))

# Generate experience data (randomly between 0.5 and 12 years)
experience <- round(runif(n, 0.5, 12), 1)

# Generate flock size data (randomly between 2 and 15 chickens)
flock_size <- sample(2:15, n, replace = TRUE)

# Create the data frame
urban_chicken_keepers <- data.frame(
  ID = 1:n,
  Gender = gender,
  Location = location,
...
head(urban_chicken_keepers)
```
Semi-structured interviews were conducted with **`r n_distinct(urban_chicken_keepers$ID)`** urban backyard chicken keepers (`r sum(urban_chicken_keepers$Gender == "Female")` female, `r sum(urban_chicken_keepers$Gender == "Male")` male,  `r sum(urban_chicken_keepers$Gender == "Non-binary")` non-binary) across three metropolitan areas (Eastern Seaboard, n=`r sum(urban_chicken_keepers$Location == "Eastern Seaboard")`; Midwest, n=`r sum(urban_chicken_keepers$Location == "Midwest")`; Pacific Northwest, n=`r sum(urban_chicken_keepers$Location == "Pacific Northwest")`). Participants had maintained backyard chickens for periods ranging from 6 months to 12 years (mean = `r; Pacific Northwest, n=15). Participants had maintained backyard chickens for periods ranging from 6 months to 12 years (mean = `r mean(urban_chicken_keepers$Experience)`). Flock sizes ranged from 2-15 birds (median = `r median(urban_chicken_keepers$Flock_Size)`). 

## Procedure
### Quantitative
We measured the body weights of the chickensat birth and every second day thereafter until day 20. They were also measured on day 21. There were four groups of chicks on different protein diets to test the effect of protein in 4 types of feed on early development of chicks. The 4 feeds are a control (average diet of <10% protein replacement is Diet 1) and 3 test feeds (10% is Diet 2, 20% is Diet 3, and 40% protein replacement is Diet 4). 
...
## Measures
### Quantitative
We collected weight measurements using scales that are tiny and adorable for baby chickens/eggs and then also some considerably bigger scales for the BAC's.

### Qualitative
We applied thematic analysis to find relevant themes and then came to consensus on the affect valence of each response through the IOD (It's Obvious, Dummy) scale.

# Results
## Chickens  
```{r}
#| label: data-analysis_setup_chicken-weight
#| echo: FALSE 
#| include: FALSE
...
```{r}
#| label: chicken_weight_growth_plot
#| echo: FALSE 
...
```
Figure 1 shows the different growth trends of the various chickens.
```{r}
#| label: chicken_weight_growth-rate
#| echo: FALSE 
#| include: false
...
print(dGrowRate)
```
Looking at diet's effect on growth rates, we found that protein content of 20% had the most positive effect on chicken growth.
```{r}
#| label: chicken_weight_growth-rate_plot
#| echo: FALSE 
...
```
As shown in Figure 2 we see that diet 3 has chickens with the highest mean growth rates (per day). Diet 4 comes in second, then diet 2, then diet 1.
```{r}
#| label: chicken_weight_regression
#| echo: FALSE 
#| include: false
...
```{r}
#| label: chicken_weight_regression-plot
#| echo: FALSE
...
```
We found that both time and diet are significant predictors of weight. Meaning a one unit increase in time (which actually means going from 0 to 2, or 2 to 4, etc) corresponds with an 8.77 gram increase in weight. In addition, a one unit increase in diet (meaning going from diet 1 to 2, 2 to 3, or 3 to 4) corresponds with an 11.78 gram increase in weight. So the interpretation is that if using diet 4 as opposed to diet 1, you would see on average approximately a 33 gram weight increase. 

## Eggs 
As shown in Table 1, we found that a component of our hypothesis was true: Big-Ass Chickens lay a Big-Ass-load of eggs.
```{r}
#| label: chicken_egg_size_weight
#| echo: FALSE 
#| include: false

# Format the table with kable
chicken_data %>%
  kable(caption = "Chicken Data: Egg Size and Weight by Breed") %>%
  kable_styling(
...
```{r}
#| label: chicken-egg_summary_stats
#| echo: FALSE
...
# Format the table for PDF output using LaTeX - wanted to ry out something that was mentioned in class but that we didnt really cover 
chicken_data %>%
...
As shown in Table 2 and contrary to our hypothesis, however, there is a negative correlation between chicken weight and egg production within the Big-Ass Chicken demographic. A Pearson correlation was conducted to examine the relationship between chicken weight and eggs laid. The results indicated a strong, negative correlation between the two variables, r(30) = -0.93, p = .02. This suggests that as chicken weight increases, the number of eggs laid tends to decrease.
```{r}
#| label: chicken-egg_correlation
#| echo: FALSE
...
# relevant values from the correlation result
...
However, as you can see in Figure 4, larger BAC's produced larger eggs, so really our main hypothesis of more chicken = more egg is like actually totally right when you think about it. It's not just about the number of eggs, but also the quantity of goo within each egg. Put another way, it's not just the size that matters, but how you use it.... it being the egg goo.
```{r}
#| label: chicken-egg_correlation_plot
#| echo: FALSE
#| fig-cap: Average Egg Weight by Chicken Breed and Egg Size
...
ggplot(egg_summary, aes(x = Breed, y = Average_Egg_Weight, fill = Egg_Size)) +
  ...
  scale_fill_manual(values = c("Small" = "red", "Medium" = "yellow", "Large" = "green", "X-Large" = "blue")) +  # Custom colors
  labs(x = "Chicken Breed", 
